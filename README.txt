000000*****************************************************************
000000* 業務仕様説明（最新版・ソースコード完全準拠・改訂版）
000000*---------------------------------------------------------------*
000000*    UPDATE         :                                           
000000*        2026/01/19 : 初期化     
000000*---------------------------------------------------------------*
...
000000*   ・満期／前倒し精算処理（FUNCTION-002）
000000*
000000* を実行する COBOL + DB2 バッチプログラムである。
000000*
000000* FUNCTION-001 は表示・確認用のプレビュー処理であり、
000000* DB 更新は一切行わない。
000000*
000000* FUNCTION-002 においてのみ、
000000* 精算処理および DB 更新を行う。
000000*
000000* 実行対象の処理は JCL パラメータ（PARAM）により制御される。
000000*
000000*   PARAM = '1' : FUNCTION-001 のみ実行
000000*   PARAM = '2' : FUNCTION-002 のみ実行
000000*   PARAM = '3' : FUNCTION-001 実行後、FUNCTION-002 を続行
000000*---------------------------------------------------------------*
000000* 対象テーブル
000000*---------------------------------------------------------------*
...
000000* 共通仕様（SRC準拠）
000000*---------------------------------------------------------------*
000000* ・処理対象は STATUS = '1' のレコードのみ
000000* ・カーソルにより 1 件ずつ処理する
000000* ・SQLCODE 判定ルール
000000*     SQLCODE = 0   ：正常
000000*     SQLCODE = 100 ：FETCH 終了（正常終了）
000000*     SQLCODE < 0   ：異常
000000*
000000* ・SQLCODE < 0 の場合は即時異常終了
000000* ・利息計算結果は整数（小数点以下切り捨て／ROUND は SRC 定義に従う）
...
000000*
000000*     INTEREST = MONEY_ROOT × 定期利率 × 契約日数 ÷ 365
000000*
...
000000*     ・エラー発生処理名を CST-ABEND-BREAKPOINT に設定
000000*     ・エラーメッセージ出力
000000*     ・ROLLBACK 実行
000000*     ・異常終了（STOP RUN）
000000*
...
000000* 注意事項
000000*---------------------------------------------------------------*
000000* ・利率は必ず DB_INTEREST_INFO から取得する
000000* ・利息計算は単利計算とする
000000* ・COMMIT は全処理正常終了時に 1 回のみ実行
000000* ・本仕様はソースコード（SRC）を正とする
000000*****************************************************************